...
int main()
{
	...
	// зчитуємо відрізки з файлу
	std::ifstream file("/home/oleksandryemets/Documents/University/GeometryJulia/SweepLine/segments");

	    std::string line;

    std::regex pattern(R"(([A-Z])\((-?\d+(?:\.\d+)?),(-?\d+(?:\.\d+)?)\))");
    ...
    while (std::getline(file, line)) {
        ...
        if (it != end) {
            ...
            if (it != end) {
                matches = *it;
                name = matches[1].str()[0];
                x = std::stod(matches[2].str());
                y = std::stod(matches[3].str());
	...
	// segments.push_back(Segment(Point('A',3, 2), Point('B',-4,-5)));
	// segments.push_back(Segment(Point('C',-7.5, 2.5),   Point('D',0,-5)));

	intersect(segments, intersections, false);

	// пишемо результати в консоль
	...
	// пишемо результати в файл
	std::ofstream out("intersection_points");
